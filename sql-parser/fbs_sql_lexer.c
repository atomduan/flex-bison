...
int main(int argc,char **argv)
{
    ...
    FILE *logger= ctxp->logger;
    YYSTYPE yylval;
    YYLTYPE yylloc;
    int yychar = 0;
fbs_read_next:
    fprintf(logger, "fbs_sql_lexer>>  ---------   parse next token  ---------  \n");
    yychar = _yylex(&yylval,&yylloc,ctxp,yyscanner);
    if (yychar <= YYEOF) {
        fprintf(logger, "fbs_sql_lexer>> Now at end of input.\n");
        goto fbs_lex_fin;
    }
    fprintf(logger, "fbs_sql_lexer>> yychar is numeric value is : %d\n\n", yychar);
    goto fbs_read_next;
fbs_lex_fin:
